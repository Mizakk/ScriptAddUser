...
$Users = Import-Csv -Delimiter "," -Path "C:\listeutilisateur.csv"            
...
{            
    ...
    $OU = "OU=" + $User.Departement + "," + "dc=VMI,dc=local"      
    $SAM = $Prenom+$Nom            
    $UPN = $Prenom + "." + $Nom + "@" + "VMI.local"
    echo $UPN         
    $Departement = $User.Departement     
    $Password = $User.MotDePasse       
    New-ADOrganizationalUnit -Name $User.Departement -Path "DC=VMI,DC=local" -ProtectedFromAccidentalDeletion $false
